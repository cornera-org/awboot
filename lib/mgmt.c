...
static uint8_t mgmt_read(sunxi_usart_t *usart, mgmt_packet_t *response)
{
	...
	return crc == response->crc;
}

// uint8_t mgmt_get_time(sunxi_usart_t *usart, uint8_t *buf)
// {
// 	uint32_t	   crc;
// 	mgmt_packet_t *p = (mgmt_packet_t *)buf;
// 	memset(buf, 0, sizeof(mgmt_packet_t));
// 	p->magic = MAGIC_REQ;
// 	p->cmd	 = CMD_GET_DATE;
// 	mgmt_write(usart, p);
// 	mgmt_read(usart, p);
// 	if (p->cmd != CMD_GET_DATE) {
// 		warning("MGMT: Time CMD error\r\n");
// 		return 1;
// 	}

// 	// RTC date starts 01/01/1980
// 	RTCDateTime	*mtime = (RTCDateTime *)p->data;
// 	struct rtc_time ctime;
//   memset(&ctime, 0, sizeof(struct rtc_time));

// 	ctime.tm_sec   = (mtime->millisecond / 1000) % 60;
// 	ctime.tm_min   = (mtime->millisecond / 60000) % 60;
// 	ctime.tm_hour  = (mtime->millisecond / 60000) / 3600;
// 	ctime.tm_year  = mtime->year + 1980;
// 	ctime.tm_mon   = mtime->month;
// 	ctime.tm_mday  = mtime->day;
// 	ctime.tm_isdst = mtime->dstflag;
// 	ctime.tm_wday  = mtime->dayofweek;

// 	debug("RTC: Time is %02d/%02d/%02d %02d:%02d:%02d\r\n", ctime.tm_year, ctime.tm_mon, ctime.tm_mday, ctime.tm_hour,
// 		  ctime.tm_min, ctime.tm_sec);
//   rtc_set_datetime(&ctime);

//   memset(&ctime, 0, sizeof(struct rtc_time));
// 	rtc_get_datetime(&ctime);
// 	debug("RTC: Time is %02d/%02d/%02d %02d:%02d:%02d\r\n", ctime.tm_year, ctime.tm_mon, ctime.tm_mday, ctime.tm_hour,
// 		  ctime.tm_min, ctime.tm_sec);
...
uint8_t mgmt_get_slot(sunxi_usart_t *usart, uint8_t *buf)
{
	...
	mgmt_write(usart, p);
	mgmt_read(usart, p);
	crc = crc32(CRC32_INITIAL, buf, sizeof(mgmt_packet_t) - sizeof(p->crc));
	if (p->crc != crc) {
		return 1;
	}
	for (i = 0; i <= 2; i++) {
		slot_msg_t *s = (slot_msg_t *)p->data;
		if (s[i].active) {
