...
{
	__asm__ __volatile__("mcr p15, 0, %0, c14, c2, 0" : : "r"(tval));
}

static void __secure cp15_write_cntp_ctl(u32 val)
{
	__asm__ __volatile__("mcr p15, 0, %0, c14, c2, 1" : : "r"(val));
}
...
static u32 __secure cp15_read_cntp_ctl(void)
{
	...
	__asm__ __volatile__("mrc p15, 0, %0, c14, c2, 1" : "=r"(val));
...
{
	__asm__ __volatile__("mcr p15, 0, %0, c1, c1, 0" : : "r"(scr));
	isb();
}
